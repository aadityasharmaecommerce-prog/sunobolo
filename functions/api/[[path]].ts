  ...
  try {
    ...
    if (adminMatch && route.startsWith('/admin/')) {
      ...
      if (memberDetailMatch && method === 'GET') {
        ...
        return json({ member: mu, subscriptions: subs?.results || [], payments: pay?.results || [], progressCount: prog?.n || 0 });
      }
      ...
      if (adminRoute === 'reports' && method === 'GET') {
        ...
        return json({ revenueByDay: rev?.results || [], subsByPlan: plan?.results || [], newUsers: users?.results || [], topCourses: courses?.results || [], activevsExpired: ae?.results || [] });
      }

      return err('Admin endpoint not found', 404);
      } catch (e: any) {
        return json({ error: 'Admin API error: ' + (e.message || 'unknown') }, { status: 500 });
      }
    }
