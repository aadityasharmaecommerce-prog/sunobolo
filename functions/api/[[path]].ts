 ...
 *
 *   POST /api/payment/create-order             - create Razorpay order (placeholder)
 *   POST /api/payment/verify                   - verify Razorpay payment (placeholder)
 *   POST /api/payment/webhook                  - Razorpay webhook (placeholder)
 *
...
/** Hash a token for storage comparison. Uses SubtleCrypto when available. */
...
/** Authenticate user from session cookie. Returns null if not authenticated. */
...
/** Check if user has active subscription (3-month access). */
...
async function handleAuthGoogle(
  ...
  // Support Google Identity Services (GIS) — receives a JWT credential
  if (body.credential) {
    // Decode the JWT to get user info (verify signature with Google's public key in production)
  ...
  // Support OAuth code exchange flow
  if (body.code && env.GOOGLE_CLIENT_ID && env.GOOGLE_CLIENT_SECRET) {
    try {
      ...
      await env.DB.prepare(
        `INSERT INTO users (id, google_id, email, name, avatar_url, last_login_at, updated_at)
         ...
         ON CONFLICT(google_id) DO UPDATE SET
           ...
           avatar_url = excluded.avatar_url, last_login_at = datetime('now'),
           updated_at = datetime('now')`
      ...
      return err('Google auth failed', 500);
    }
  }
...
async function handleAuthMe(
  ...
  if (subscribed) {
    subscriptionDetails = await env.DB.prepare(
      ...
      .bind(user.id)
      .first<{ started_at: string; expires_at: string }>();
  }
  ...
  return json({
    ...
    subscription: {
      active: subscribed,
      ...(subscriptionDetails ? { started_at: subscriptionDetails.started_at, expires_at: subscriptionDetails.expires_at } : {}),
    },
  });
}
  ...
  if (subscribed) {
    details = await env.DB.prepare(
      `SELECT started_at, expires_at FROM subscriptions
       WHERE user_id = ? AND status = 'active'
       ORDER BY expires_at DESC LIMIT 1`
    ).bind(user.id).first<{ started_at: string; expires_at: string }>();
  }
...
// ── Payment placeholders (Razorpay not yet configured) ──
...
async function handlePaymentCreateOrder(
  ...
  const amount = 59900; // ₹599 in paise

  if (!env.RAZORPAY_KEY_ID || !env.RAZORPAY_KEY_SECRET) {
    // Razorpay not configured — return demo order for testing
    ...
    await env.DB.prepare(
      `INSERT INTO payments (id, user_id, razorpay_order_id, amount, currency, status)
       VALUES (?, ?, ?, ?, 'INR', 'created')`
    ).bind(crypto.randomUUID(), user.id, orderId, amount).run();
  ...
  // Real Razorpay order creation (when configured)
  try {
    ...
    const res = await fetch('https://api.razorpay.com/v1/orders', {
      ...
      body: JSON.stringify({
        amount,
        currency: 'INR',
        receipt: `sb_${user.id.slice(0, 16)}_${Date.now()}`,
      }),
    });
...
async function handlePaymentVerify(
  ...
  if (!env.RAZORPAY_KEY_SECRET) {
    ...
    await activateSubscription(user.id, body.razorpay_order_id, body.razorpay_payment_id, env.DB);
    return json({ verified: true, demo: true });
  }

  // Real verification with Razorpay signature
  try {
    const crypto_ = await import('crypto');
    const expectedSig = crypto_
    ...
    if (expectedSig !== body.razorpay_signature) {
      ...
      ).bind(body.razorpay_order_id).run();
...
async function activateSubscription(
  ...
  const existing = await db
    .prepare(`SELECT id FROM payments WHERE razorpay_payment_id = ? AND status = 'paid'`)
    .bind(paymentId)
    .first();
  if (existing) return;
  ...
  await db
    .prepare(
      `UPDATE payments SET status = 'paid', verified_at = datetime('now')
...
async function handlePaymentWebhook(
  ...
  // Razorpay webhook — verify signature, handle payment events
  // Placeholder: accept and acknowledge
  const body = await request.json<Record<string, unknown>>().catch(() => ({}));
  const event = body.event as string | undefined;

  if (event === 'payment.captured' || event === 'payment.authorized') {
    // Handle payment success — in production, verify webhook signature first
  }
...
export const onRequest: PagesFunction<Env> = async ({ request, env, params }) => {
  ...
  try {
    ...
    // Courses — add access info based on subscription
    if (route === '/courses' && method === 'GET') {
      ...
      const coursesWithAccess = (results || []).map((course: any) => ({
        ...course,
        locked: !course.is_free && !subscribed,
      }));

      return json(coursesWithAccess);
    }
    ...
    if (route === '/payment/create-order' && method === 'POST') {
      return handlePaymentCreateOrder(request, env);
    }
    ...
    if (route === '/payment/webhook' && method === 'POST') {
