...
export const onRequest: PagesFunction<Env> = async ({ request, env, params }) => {
  ...
  try {
    ...
    // ── Admin API (server-side auth required) ──
    const adminMatch = route.match(/^\/admin\/(.+)/);
    if (adminMatch && route.startsWith('/admin/')) {
      const user = await authenticateUser(request, env);
      if (!user) return err('Login required', 401);
      const adminUser = await env.DB.prepare('SELECT is_admin FROM users WHERE id = ?').bind(user.id).first<{ is_admin: number }>();
      if (!adminUser || !adminUser.is_admin) return err('Admin access required', 403);
      const adminRoute = adminMatch[1];

      // GET /api/admin/dashboard
      if (adminRoute === 'dashboard' && method === 'GET') {
      ...
      if (adminRoute === 'members' && method === 'GET') {
        ...
        const q = `SELECT u.id, u.name, u.email, u.created_at, u.is_admin, u.google_id,
          s.package_id as plan, s.status as sub_status, s.started_at, s.expires_at
          FROM users u LEFT JOIN subscriptions s ON u.id = s.user_id AND s.id = (
        ...
        const countQ = `SELECT COUNT(*) as n FROM users u LEFT JOIN subscriptions s ON u.id = s.user_id WHERE ${where}`;
        const count = await env.DB.prepare(countQ).bind(...params).first<{ n: number }>();
        return json({ members: results || [], total: count?.n || 0, page, limit });
      }

      // GET /api/admin/members/:id
      const memberDetailMatch = adminRoute.match(/^members\/([\w-]+)$/);
      if (memberDetailMatch && method === 'GET') {
        const uid = memberDetailMatch[1];
        const user = await env.DB.prepare('SELECT id, name, email, phone, avatar_color, is_admin, created_at, google_id FROM users WHERE id = ?').bind(uid).first();
        ...
        const payments = await env.DB.prepare('SELECT * FROM payments WHERE user_id = ? ORDER BY created_at DESC').bind(uid).all();
        const progress = await env.DB.prepare('SELECT COUNT(*) as n FROM user_progress WHERE user_id = ?').bind(uid).first<{ n: number }>();
        return json({ member: user, subscriptions: subs?.results || [], payments: payments?.results || [], progressCount: progress?.n || 0 });
      }

      // GET /api/admin/payments
      if (adminRoute === 'payments' && method === 'GET') {
        const url = new URL(request.url);
        const search = url.searchParams.get('search') || '';
        ...
        const { results } = await env.DB.prepare(q).bind(...params, limit, offset).all();
        const countQ = `SELECT COUNT(*) as n FROM payments p LEFT JOIN users u ON p.user_id = u.id WHERE ${where}`;
        const count = await env.DB.prepare(countQ).bind(...params).first<{ n: number }>();
        ...
        return json({ payments: results || [], total: count?.n || 0, page, limit, summary });
      }

      // GET /api/admin/reports
      if (adminRoute === 'reports' && method === 'GET') {
        const url = new URL(request.url);
        const range = url.searchParams.get('range') || '30d';
        let dateFilter = "date('now', '-30 days')";
        if (range === '7d') dateFilter = "date('now', '-7 days')";
        if (range === '1m') dateFilter = "date('now', '-1 month')";
        if (range === 'all') dateFilter = "date('now', '-100 years')";
        const [revenueByDay, subsByPlan, newUsers, topCourses, activevsExpired] = await Promise.all([
          env.DB.prepare(`SELECT date(created_at) as day, SUM(amount) as revenue, COUNT(*) as count FROM payments WHERE status = 'paid' AND created_at >= ${dateFilter} GROUP BY date(created_at) ORDER BY day`).all(),
          ...
          env.DB.prepare(`SELECT date(created_at) as day, COUNT(*) as count FROM users WHERE created_at >= ${dateFilter} GROUP BY date(created_at) ORDER BY day`).all(),
        ...
        return json({ revenueByDay: revenueByDay?.results || [], subsByPlan: subsByPlan?.results || [], newUsers: newUsers?.results || [], topCourses: topCourses?.results || [], activevsExpired: activevsExpired?.results || [] });
      }
